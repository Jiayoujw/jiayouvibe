...
interface ShareButtonsProps {
  /** Full URL to share */
  url: string
  /** Title for the shared page */
  title: string
  /** Description / summary */
  description?: string
}

/* ---- Inline brand SVGs (not available in lucide-react v1.17) ---- */
...
const channels: ShareChannel[] = [
  {
    name: 'Twitter / X',
    icon: X,
    color: 'hover:bg-slate-700 hover:text-slate-100',
    buildUrl: ({ url, title }) =>
...
export default function ShareButtons({ url, title, description }: ShareButtonsProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      <span className="text-xs font-jetbrains uppercase tracking-wider text-slate-500 mr-1">
        分享
      </span>
      ...
      {channels.map((ch) => (
        <button
          ...
          onClick={() =>
            window.open(
              ch.buildUrl({ url, title, description }),
              'share',
              'width=600,height=400,noopener,noreferrer',
            )
          }
          aria-label={`分享到 ${ch.name}`}
          className={cn(
