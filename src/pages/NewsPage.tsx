  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 sm:py-12">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm font-jetbrains max-w-2xl">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-6 space-y-3">
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-200 border ${
                    activeCategory === tab.key
                      ...
                      : 'border-slate-700/60 text-slate-400 hover:text-slate-200 hover:border-slate-600 hover:bg-slate-800/40'
                  }`}
                >
                  {activeCategory === tab.key && (
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-700/60 bg-slate-900/60 text-slate-200 text-sm placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/30 transition-all duration-200"
              />
            </div>
          </div>

          {/* Results Info */}
          <div className="flex items-center gap-2 text-xs text-slate-500 font-jetbrains mb-6">
          ...
          {filteredNews.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-slate-500">
              <Newspaper className="h-10 w-10 mb-3 text-slate-600" />
        ...
        <div className="w-full lg:w-72 shrink-0 space-y-6">
          ...
          <div className="rounded-2xl border border-slate-800/80 bg-slate-900/60 backdrop-blur-sm p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-sora font-semibold text-slate-200">
                热门标签
              </h3>
            </div>
            <div className="flex flex-wrap gap-2">
              {tagCloud.map(([tag, count]) => (
                <button
                  ...
                  className="px-2.5 py-1 text-xs rounded-lg border border-slate-700/50 bg-slate-800/40 text-slate-400 hover:text-cyan-300 hover:border-cyan-500/40 hover:bg-cyan-500/10 transition-all duration-200"
                >
                  {tag}
                ...
                <li key={item.id} className="flex gap-3">
                  ...
                  <div className="min-w-0">
                    <a
                      ...
                      className="text-xs text-slate-300 hover:text-cyan-400 transition-colors line-clamp-2 leading-snug"
                    >
                      {item.title}
                    </a>
                    <p className="text-[10px] text-slate-600 font-jetbrains mt-0.5">
...
function NewsCard({ item }: { item: NewsItem }) {
  return (
    <div className="relative pl-8 group">
      ...
      {/* Date Label */}
      <div className="mb-2 text-xs font-jetbrains text-slate-500">
        {formatDisplayDate(item.date)}
      </div>

      {/* Card */}
      <div
        className="rounded-2xl border border-slate-800/80 bg-slate-900/60 backdrop-blur-sm p-5
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            >
              {item.title}
              <ExternalLink className="inline h-3.5 w-3.5 ml-1.5 -mt-0.5 text-slate-600 group-hover/link:text-cyan-400 transition-colors" />
            </a>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            {/* Source Badge */}
            <span className="px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-slate-800 text-slate-400 border border-slate-700/50">
              {item.source}
            </span>
            ...
            <div className="flex flex-wrap gap-1.5">
              {item.tags.slice(0, 4).map((tag) => (
                <span
                  key={tag}
                  className="px-2 py-0.5 text-[10px] rounded-md bg-slate-800/60 text-slate-500 border border-slate-700/40"
                >
                  #{tag}
                </span>
              ))}
            ...
            {/* Reading Time */}
            <div className="ml-auto flex items-center gap-1 text-[11px] text-slate-600 font-jetbrains">
